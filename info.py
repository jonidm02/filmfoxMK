...
']
BOT_TOKEN = environ['BOT_TOKEN', "7375736066:AAFJfNCSQH-k4Oo0gAjx2_nyyDSHlzBaqYs"]
...
NOR_IMG = environ.get("NOR_IMG", "https://graph.org/file/f0d3bb4d50fcde32a6fc3-7a099c833b38c31462.jpg")
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002417496242').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1496229374').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL','-1002398914394')
auth_grp = environ.get('AUTH_GROUP', '-1002170681398')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002170681398')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002398914394')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None
NO_RESULTS_MSG = is_enabled((environ.get("NO_RESULTS_MSG", 'False')), False)

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://filmfoxmk:CyYbxcvsasmtHgdR@cluster0.xxwgp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0
")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002282413675'))
